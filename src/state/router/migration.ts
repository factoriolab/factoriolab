...
import { ZARRAYSEP, ZEMPTY, ZFIELDSEP, ZTRUE } from './constants';
import { LabParams } from './lab-params';
import { Params } from './params';
import { Zip } from './zip';
import { ZipVersion } from './zip-version';

const V10LISTSEP = '_'; // V11: Deprecated
const V10NULL = '?'; // V11: Deprecated
const V10EMPTY = '='; // V11: _
...
export class Migration {
  ...
  private migrateV0(state: MigrationState): MigrationState {
    ...
    if (params[ZipSectionV10.Settings]) {
      ...
      const displayRateV0 = this.zip.parseNumber(s[6]) ?? 60;
      const displayRateV1 =
        displayRateV0 === 1 ? '0' : displayRateV0 === 3600 ? '2' : '1';

      params[ZipSectionV10.Settings] = this.zip.zipFields([
        modId,
        displayRateV1,
  ...
  private migrateV1(state: MigrationState): MigrationState {
    ...
    if (params[ZipSectionV10.Settings]) {
      ...
      if (s.length > index) {
        ...
        const expensive = this.zip.parseBool(val[0]);
        if (expensive) {
          warnings.push(MigrationWarning.ExpensiveDeprecation);
        }
      }

      params[ZipSectionV10.Settings] = this.zip.zipFields(s);
    }
    ...
    if (params[ZipSectionV10.Recipes]) {
      ...
      for (const recipe of list) {
        ...
        if (s.length > index) {
          ...
          s[index] = this.zip.zipString(numString);
        }

        migrated.push(this.zip.zipFields(s));
      }
    ...
    if (params[ZipSectionV10.Machines]) {
      ...
      for (const machine of list) {
        ...
        if (s.length > index) {
          ...
          s[index] = this.zip.zipString(asString);
        }
    ...
    if (params[section]) {
      ...
      for (const s of list.map((z) => z.split(ZFIELDSEP))) {
        ...
        if (s.length > countIndex) {
          ...
          s[countIndex] = this.zip.zipArray([beacons.length]);

          // Add beacon settings
          beacons.push(
            this.zip.zipFields([
              count,
              this.zip.zipString(moduleIds),
              this.zip.zipString(id),
              this.zip.zipString(total),
            ]),
          );
        }
  ...
  private migrateToV8(state: MigrationState): MigrationState {
        ...
        if (o[2] === '3') {
          if (isBare) {
            ...
            if (o.length > 3) {
              ...
              recipes.push(this.zip.zipFields(maximize));
              recipes.push(this.zip.zipFields(limit));
              needsLimitDeprecationWarning = true;
            } else {
              recipes.push(this.zip.zipFields([o[0], o[1]]));
            }
             ...
             */
            o[2] = '0';
            migrated[i] = this.zip.zipFields(o);
          }
        } else {
          if (o.length > 3) {
            ...
            const maximize = [o[0], '1', '', ObjectiveType.Maximize.toString()];
            migrated[i] = this.zip.zipFields(maximize);
            migrated.push(this.zip.zipFields(limit));
            needsLimitDeprecationWarning = true;
          }
        }
      }
    ...
    if (params[ZipSectionV10.Settings]) {
      ...
      if (s.length > 2 + x) {
        const disabledRecipeIds = this.zip.parseArray(
          ...
          for (const id of disabledRecipeIds) {
            ...
            for (let i = 0; i < list.length; i++) {
              ...
              if (r[0] === id) {
                found = true;
                r[1] = ZTRUE;
                list[i] = this.zip.zipFields(r);
              }
            }

            if (!found) {
              list.push(this.zip.zipFields([id, ZTRUE]));
            }
          }
    ...
    if (params[section]) {
      ...
      for (const s of list.map((z) => z.split(ZFIELDSEP))) {
        if (s.length > index) {
          ...
          const moduleIds = this.zip.parseArray(moduleIdsStr);
          ...
          s[index] = this.zip.zipArray(moduleIndices);
        ...
        .forEach((s, i) => {
          ...
          if (s[0] !== V10EMPTY && s[0] !== '') {
            if (s.length > 1) {
              ...
              if (moduleIdsStr) {
                ...
                s[1] = this.zip.zipArray([modules.length]);
                modules.push(this.zip.zipFields(['', moduleId]));
              }
            }
          }
          ...
          if (s.length > 2) {
            ...
            beacons.push(
              this.zip.zipFields([
                count,
                this.zip.zipArray(moduleIndices),
                this.zip.zipString(id),
              ]),
            );
          }
  ...
  private migrateV10(state: MigrationState): MigrationState {
    ...
    // Modules
    const oldModules = params[ZipSectionV10.Modules] as string | undefined;
    delete params[ZipSectionV10.Modules];
    const newModules = oldModules?.split(V10LISTSEP);

    // Beacons
    const oldBeacons = params[ZipSectionV10.Beacons] as string | undefined;
    delete params[ZipSectionV10.Beacons];
    const newBeacons = oldBeacons?.split(V10LISTSEP);

    // Objectives
    const oldObjectives = params[ZipSectionV10.Objectives] as
      | string
      | undefined;
    ...
    if (checkedObjectiveIds.size) {
      params['och'] = this.zip.zipDiffSubset(
        checkedObjectiveIds,
        undefined,
        objectiveIds,
      );
    ...
    // Items
    const oldItems = params[ZipSectionV10.Items] as string | undefined;
    ...
    // Recipes
    const oldRecipes = params[ZipSectionV10.Recipes] as string | undefined;
    ...
    // Machines
    const oldMachines = params[ZipSectionV10.Machines] as string | undefined;
    ...
    // Settings
    const oldSettings = params[ZipSectionV10.Settings] as string | undefined;
    ...
    if (newMod && !state.isBare)
      newMod = datasets.modHash[this.compression.idToN(newMod)];
  ...
  restoreV10ResearchedTechnologies(
    ...
    const techRecord = toRecord(technologies);
    let addIds: Set<string>;
    do {
      addIds = new Set<string>();

      for (const id of selection) {
        const tech = techRecord[id].technology;
        tech?.prerequisites
