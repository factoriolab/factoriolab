...
import { ItemsState, ItemsStore } from '../store/items.store';
import { MachinesState, MachinesStore } from '../store/machines.store';
import { ObjectivesState, ObjectivesStore } from '../store/objectives.store';
import { RecipesState, RecipesStore } from '../store/recipes.store';
import {
...
export class RouterService {
  ...
  recipesStr = inject(RecipesStore);
  ...
  constructor() {
    ...
    effect(() => {
      ...
      const hash = this.settingsStr.hash();
      if (!ready || !hash) return;
      const objectives = this.objectivesStr.state();
      const items = this.itemsStr.state();
      const recipes = this.recipesStr.state();
      const machines = this.machinesStr.state();
      const settings = this.settingsStr.state();
  ...
  dispatch(state: PartialState): void {
    this.objectivesStr.load(state.objectivesState);
    this.itemsStr.load(state.itemsState);
    this.recipesStr.load(state.recipesState);
    this.machinesStr.load(state.machinesState);
    this.settingsStr.load(state.settingsState);
    this.ready.set(true);
  }
