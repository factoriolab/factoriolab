...
export class ObjectivesStore extends EntityStore<ObjectiveState> {
  ...
  objectives = computed(() => {
    ...
    const recipesState = this.recipesStr.settings();
    const machinesState = this.machinesStr.settings();
    const settings = this.settingsStr.settings();
    const data = this.recipesStr.adjustedDataset();
  ...
  normalizedObjectives = computed(() => {
    ...
    const itemsState = this.itemsStr.settings();
    const beltSpeed = this.settingsStr.beltSpeed();
    const dispRateInfo = this.settingsStr.displayRateInfo();
    const data = this.recipesStr.adjustedDataset();
  ...
  matrixResult = computed(() => {
    ...
    const settings = this.settingsStr.settings();
    const data = this.recipesStr.adjustedDataset();
    const paused = this.preferencesStr.paused();
  ...
  stepsModified = computed(() => {
    ...
    const objectives = this.baseObjectives();
    const itemsState = this.itemsStr.state();
    const recipesState = this.recipesStr.state();
  ...
  effectivePowerUnit = computed(() => {
    const powerUnit = this.preferencesStr.powerUnit();
  ...
  recipesModified = computed(() => {
    const state = this.recipesStr.state();
    const objectives = this.baseObjectives();
  ...
  constructor() {
    ...
    this.settingsStr.load$
      .pipe(
        switchMap(() =>
          toObservable(this.settingsStr.displayRate, { injector }).pipe(
            pairwise(),
