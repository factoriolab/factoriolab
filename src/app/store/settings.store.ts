...
export class SettingsStore extends Store<SettingsState> {
  ...
  datasetsStr = inject(DatasetsStore);
  ...
  mod = computed(() => {
    ...
    const entities = this.datasetsStr.modEntities();
  ...
  hash = computed(() => {
    const modId = this.modId();
    if (modId == null) return undefined;
    const datasets = this.datasetsStr.state();
  ...
  i18n = computed(() => {
    ...
    if (modId == null) return undefined;
    const datasets = this.datasetsStr.state();
    const lang = this.preferencesStr.language();
  ...
  modStates = computed(() => {
    ...
    const states = this.preferencesStr.states();
  ...
  columnsState = computed(() => {
    const data = this.dataset();
    const columns = this.preferencesStr.columns();
  ...
  computeDataset(
    ...
    const categoryEntities = toEntities(coalesce(mod?.categories, []));
    const iconFile = `data/${coalesce(mod?.id, DEFAULT_MOD)}/icons.webp`;
    const iconEntities = toEntities(coalesce(mod?.icons, []));
    const itemData = toEntities(coalesce(mod?.items, []));
    const recipeData = toEntities(coalesce(mod?.recipes, []));
    const limitations = reduceEntities(coalesce(mod?.limitations, {}));
    const locationEntities = toEntities(coalesce(mod?.locations, []));
